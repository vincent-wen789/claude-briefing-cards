...
Claude Code のデスクトップアプリは、会話の中にライブ HTML ウィジェットを描画できます——ほとんどの人は、たまのグラフ表示でしか見かけません。本プラグインはこれを会話そのものに使います。エージェントが決定・進捗確認・一括承認を必要とするとき、文字の壁ではなくカードを描きます。カードのボタンが構造化された返信をチャットに送り返すので、「ユーザーが実際に承認したのはどの選択肢か」を推測する必要がなくなります。**ボタンのクリックは、あなたがその返信を自分で打つのと同じ**——本文がそのままチャットに現れ、何も黙って送られず、いつでもボタンを無視して打ち込めます。

> ℹ️ **お断り——これは個人版です。** 誰でもインストールして動きますが、常駐するルーティング指示は中国語で、特定ユーザー向けに調整されており、既定で「積極的にカードを描く」傾向です(スロットルが効くので氾濫はしません)。1 箇所の編集で姿勢全体を保守的な英語に切り替えられます——[チューニング](#チューニング)参照。

## なぜ存在するのか

エージェントの報告には、2 つの慢性的な失敗モードがあります。

1. **不完全。** 散文はエージェントに、都合の悪い部分——トレードオフ、決めないことのコスト、自らの推奨——をこっそり省かせます。名前付きブロックを持つ固定テンプレートは、それらを黙って省きにくくします。
...
- **すでに処理済みのホストの癖** — どの規則も、まず失敗にぶつかって学んだもの。あなたが同じ轍を踏まずに済みます:inline `onclick` 禁止(ストリーミング中に壊れる)、ペイロードは `data-send` 属性へ、状態の見た目は JS で inline style として書く(`<style>` ブロックは黙って効かない)、プログラム的な `.click()` 禁止(信頼されないイベントは破棄される)、スクリプト読み込みまでコントロールは無効化、提出後は目に見えるロック。
...
- **Claude Code デスクトップアプリ** — チャット内ウィジェットツール(`mcp__visualize__show_widget` と `sendPrompt()` ブリッジ)を標準搭載。他に入れるものはありません——追加の MCP サーバーも API キーも設定も不要。チャットでウィジェットやグラフが描画されたのを見たことがない? その表示層はデスクトップアプリ専用です——すぐ下の ⚠️ を参照。
...
アンインストールはいつでも `/plugin uninstall briefing-card@vincent-plugins`(または `/plugin` メニュー)で——マシン上の他のものには一切触れません。
...
本プラグインは **SessionStart フック**でこれを解決します。セッション開始 / clear / compact のたびに、ルーティング規則を常駐コンテキストとして注入。これは小さなブロックで——約 1.3k トークン、セッション開始時に一度だけ注入され(以降はキャッシュ)、メッセージごとに再送はされません。skill(テンプレート + 完全なルーティング仕様)は、実際にカードを描くときだけオンデマンドで読み込まれます。インストールするだけで動作し、`CLAUDE.md` を編集する必要はありません。
...
冒頭で触れたとおり、ボタンのクリックは、あなたがそのメッセージを自分で打つのと完全に等価です。ペイロード全文がチャットに現れ、何も黙って送られず、ボタンを無視して返信を打ち込むこともいつでもできます。

## チューニング

ダイヤルは 2 つ、どちらも `hooks/session-context.md`(注入されるブロック)の中にあります。

- **トリガーの姿勢。** 本エディションは**既定でカードを出す**設定で出荷されています——報告 / 決定 / 承認のターンは、一文で済む場合を除いてカードを描きます。反転するには:`hooks/session-context.md` で、この既定を逆——「テキストのままにし、内容が本当に収まらないときだけ描く」——に差し替えます。「既定の姿勢(デフォルト反転)」と記されている段落です。
...
この個人版を広く配布用に直すには(冒頭のお断り参照):ルーティングブロック(`hooks/session-context.md`)を英語に翻訳し、既定の姿勢を保守的に反転させます(チューニング参照)——さもないと、作者以外にはカードを出しすぎます。
